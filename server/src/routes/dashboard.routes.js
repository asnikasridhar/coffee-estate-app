...
router.get('/', asyncHandler((req, res) => {
  ...
  const plantByType = rows(`SELECT COALESCE(vm.variety_name,pd.plant_type,'Unmapped') plant_type, COALESCE(SUM(pi.plant_count),0) plant_count FROM plant_inventory pi LEFT JOIN variety_master vm ON vm.variety_master_id=pi.variety_master_id LEFT JOIN plantdetails pd ON pd.plant_id=pi.plant_id WHERE pi.property_id = @propertyId GROUP BY COALESCE(vm.variety_name,pd.plant_type,'Unmapped') ORDER BY plant_count DESC`, { propertyId });
