...
router.get('/', asyncHandler((req, res) => {
  ...
  const plantInventorySummary = row(`SELECT COUNT(*) entries, COALESCE(SUM(plant_count),0) total_plants FROM plant_inventory WHERE property_id = @propertyId`, { propertyId });
  const plantByBlock = rows(`SELECT b.block_name AS name, COALESCE(SUM(pi.plant_count),0) value FROM blocks b LEFT JOIN plant_inventory pi ON pi.block_id = b.block_id AND pi.property_id = @propertyId WHERE b.property_id = @propertyId GROUP BY b.block_id, b.block_name HAVING value > 0 ORDER BY value DESC`, { propertyId });
  const plantByType = rows(`SELECT pd.plant_type AS name, COALESCE(SUM(pi.plant_count),0) value FROM plant_inventory pi JOIN plantdetails pd ON pd.plant_id = pi.plant_id WHERE pi.property_id = @propertyId GROUP BY pd.plant_id, pd.plant_type ORDER BY value DESC`, { propertyId });
  const plantBySubBlock = rows(`SELECT b.block_name, COALESCE(pi.sub_block_name,'Unspecified') AS sub_block_name, COALESCE(SUM(pi.plant_count),0) plant_count FROM plant_inventory pi JOIN blocks b ON b.block_id = pi.block_id WHERE pi.property_id = @propertyId GROUP BY b.block_name, COALESCE(pi.sub_block_name,'Unspecified') ORDER BY b.block_name, plant_count DESC`, { propertyId });
