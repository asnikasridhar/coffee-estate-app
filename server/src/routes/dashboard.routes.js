...
router.get('/', asyncHandler((req, res) => {
  ...
  const profitIncome = row(`SELECT ${money('SUM(sale_value)')} total FROM finance_sale WHERE status='confirmed' AND date(sale_date) BETWEEN date(@from) AND date(@to) AND property_id=@propertyId`, { ...profitRange, propertyId });
  const profitYield = { total: 0 };
  const profit = Number(profitIncome.total || 0) - Number(profitExpenses.total || 0);
