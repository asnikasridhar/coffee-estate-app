...
router.get('/', asyncHandler((req, res) => {
  ...
  res.json({
    ...
    plants: rows(`SELECT pd.plant_id, pd.plant_type, pd.plantdetailscol, pd.details, pd.block_id, pd.property_id FROM plantdetails pd ${propertyId ? 'WHERE pd.property_id = @propertyId' : ''} ORDER BY pd.plant_type`, { propertyId }),
    crops: rows(`SELECT crop_id, crop_name, property_id FROM crop_master ${propertyId ? 'WHERE property_id = @propertyId' : ''} ORDER BY crop_name`, { propertyId }),
    cropTypes: rows(`SELECT ct.crop_type_id, ct.crop_id, ct.type_name, ct.block_id FROM crop_type_master ct JOIN crop_master c ON c.crop_id=ct.crop_id ${propertyId ? 'WHERE c.property_id = @propertyId' : ''} ORDER BY ct.type_name`, { propertyId }),
    varieties: rows(`SELECT vm.variety_master_id, vm.variety_id, vm.crop_type_id, vm.variety_name FROM variety_master vm JOIN crop_type_master ct ON ct.crop_type_id=vm.crop_type_id JOIN crop_master c ON c.crop_id=ct.crop_id ${propertyId ? 'WHERE c.property_id = @propertyId' : ''} ORDER BY vm.variety_name`, { propertyId }),
    expenseTypes: rows('SELECT expensetype_id, expense_name, expense_code, current_rate FROM expensetype ORDER BY expense_name'),
    yieldTypes: rows(`SELECT yt.yieldtype_id, yt.yieldtype_name, yt.plant_id, pd.plant_type AS plant_name FROM yieldtype yt LEFT JOIN plantdetails pd ON pd.plant_id = yt.plant_id ${propertyId ? 'WHERE pd.property_id = @propertyId' : ''} ORDER BY yt.yieldtype_name`, { propertyId }),
    yieldRates: rows(`SELECT yr.yieldrate_id, yr.yieldtype_id, yr.yieldrate_running_rate AS rate, yr.yieldrate_code AS season, yr.baseunit_id, bu.baseunit_name, yt.yieldtype_name FROM yieldrate yr LEFT JOIN baseunit bu ON bu.baseunit_id = yr.baseunit_id LEFT JOIN yieldtype yt ON yt.yieldtype_id = yr.yieldtype_id LEFT JOIN plantdetails pd ON pd.plant_id = yr.plant_id ${propertyId ? 'WHERE pd.property_id = @propertyId' : ''} ORDER BY yr.yieldrate_code DESC, yr.yieldrate_id DESC`, { propertyId }),
