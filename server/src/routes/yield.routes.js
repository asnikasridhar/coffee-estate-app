...
router.patch('/:id', asyncHandler((req, res) => {
  ...
  if (userId && propertyId) assertPropertyAccess(userId, propertyId);
  const payload = yieldSchema.parse(req.body);
  if (propertyId && !row('SELECT yr.yieldrate_id FROM yieldrate yr JOIN plantdetails pd ON pd.plant_id = yr.plant_id JOIN blocks b ON b.block_id = pd.block_id WHERE yr.yieldrate_id = ? AND b.property_id = ?', [payload.yieldrate_id, propertyId])) throw new Error('Yield rate does not belong to selected property');
  const result = run(`UPDATE yield_settlement SET yieldrate_id = @yieldrate_id, yield_quantity = @quantity, yield_settlement_date = @picking_date, modified_by = @modified_by, modified_date = CURRENT_TIMESTAMP WHERE yield_settlement_id = @id ${propertyId ? 'AND yieldrate_id IN (SELECT yr.yieldrate_id FROM yieldrate yr JOIN plantdetails pd ON pd.plant_id = yr.plant_id JOIN blocks b ON b.block_id = pd.block_id WHERE b.property_id = @propertyId)' : ''}`, { ...payload, modified_by: req.body.modified_by || 'Admin', id: req.params.id, propertyId });
...
router.delete('/:id', asyncHandler((req, res) => {
  ...
  if (userId) requireOwner(userId);
  if (userId && propertyId) assertPropertyAccess(userId, propertyId);
  const result = run(`DELETE FROM yield_settlement WHERE yield_settlement_id = @id ${propertyId ? 'AND yieldrate_id IN (SELECT yr.yieldrate_id FROM yieldrate yr JOIN plantdetails pd ON pd.plant_id = yr.plant_id JOIN blocks b ON b.block_id = pd.block_id WHERE b.property_id = @propertyId)' : ''}`, { id: req.params.id, propertyId });
  if (!result.changes) return res.status(404).json({ error: 'Yield entry not found' });
  res.status(204).end();
}));
