...
import { createAttendance, createRainfall, createYield, deleteQuickEntry, deleteResource, loadAttendance, loadDashboard, loadMeta, loadRainfall, loadResource, loadYield, saveResource, updateQuickEntry, updateResource, login, logout, setSelectedPropertyId, registerProperty, setDashboardRanges } from './features/appSlice';
import './styles.css';
...
function emptyFor(resource:string){ const f:any = { created_by:'Admin' }; for(const x of fieldConfig[resource] || []) f[x[0]] = x[1] === 'date' ? today : x[1] === 'number' ? '0' : x[1] === 'checkbox' ? 'active' : ''; return f; }
...
const gridColumns: Record<string,string[]> = {
  ...
  laborVendors:['labor_name','vendor_name','vendor_labor_percentage','laborvendorcode'],
  wages:['labor_name','wage_fixed','wage_variable','wage_ot_perhr_price','wage_fix_code'],
  wageSettlements:['wage_name','settled_amount','advance_amount','running_wage_transaction_date'],
  vendorSettlements:['labor_vendor_name','settled_amount','advance_amount','running_wage_transaction_date'],
  yieldTypes:['yieldtype_name','plant_name'],
  yieldRates:['plant_name','yieldtype_name','yieldrate_code','yieldrate_running_rate','baseunit_name'],
  cropDetails:['property_name','yield_obtained','selling_price','other_detail'],
  fertilizers:['property_name','fertilizer_name','date_of_application','other_details'],
  assets:['property_name','asset_name','asset_price','procured_year','is_active','asset_procured_source'],
  reports:['property_name','total_expenditure','total_revenue','profit_loss'],
  expenseTypes:['expense_code','expense_name','current_rate'],
  expenses:['expense_type_name','expense_code','expense_occurence_date','other_expense']
};
function namedRows(resource:string, rows:any[], meta:any){
  const find=(source:string,id:string,value:any,name:string)=>meta?.[source]?.find((x:any)=>String(x[id])===String(value))?.[name] || '';
  return (rows||[]).map((r:any)=>({...r,
    property_name:r.property_name||find('properties','property_id',r.property_id,'property_name'),
    ...
    vendor_name:r.vendor_name||find('vendors','vendor_id',r.vendor_id,'vendorname'),
    wage_name:r.wage_name||find('wages','wage_id',r.wage_id,'wage_label'),
    labor_vendor_name:r.labor_vendor_name||find('laborVendors','laborvendor_id',r.laborvendor_id,'labor_vendor_label'),
    yieldtype_name:r.yieldtype_name||find('yieldTypes','yieldtype_id',r.yieldtype_id,'yieldtype_name'),
    baseunit_name:r.baseunit_name||find('baseUnits','baseunit_id',r.baseunit_id,'baseunit_name'),
    expense_type_name:r.expense_type_name||find('expenseTypes','expensetype_id',r.expensetype_id,'expense_name'),
    is_active:Number(r.isactive)===1?'Yes':'No'
  }));
}
...
function Attendance({meta,rows,submit,update,remove}:any){const initial=()=>({labor_id:firstId(meta?.labors,'labor_id'),entry_date:today,attendance_value:'1',created_by:'Admin'});const [f,setF]=useState(initial);const [editing,setEditing]=useState<any>(null);useEffect(()=>setF((x:any)=>({...x,labor_id:x.labor_id||firstId(meta?.labors,'labor_id')})),[meta]);const save=()=>{editing?update(editing,f):submit(f);setEditing(null);setF(initial());};const edit=(r:any)=>{setEditing(r.attendance_id);setF({labor_id:String(r.labor_id),entry_date:r.entry_date,attendance_value:String(r.attendance_value),created_by:'Admin'});};return <><Form title={`${editing?'Edit':'Add'} attendance`} onSubmit={save}><Select label="Labor" v="labor_id" f={f} setF={setF} opts={meta?.labors} id="labor_id" name="labor_name"/><Input label="Date" type="date" v="entry_date" f={f} setF={setF}/><Input label="Value" type="number" step="0.25" v="attendance_value" f={f} setF={setF}/>{editing&&<button type="button" className="secondary" onClick={()=>{setEditing(null);setF(initial());}}>Cancel edit</button>}</Form><Table rows={rows} columns={['entry_date','labor_name','attendance_value','wage','property_name','labor_cost']} onEdit={edit} onDelete={(r:any)=>remove(r.attendance_id)}/></>}
function Rainfall({meta,rows,submit,update,remove}:any){const initial=()=>({block_id:firstId(meta?.blocks,'block_id'),recorded_date:today,rain_value:'0',created_by:'Admin'});const [f,setF]=useState(initial);const [editing,setEditing]=useState<any>(null);useEffect(()=>setF((x:any)=>({...x,block_id:x.block_id||firstId(meta?.blocks,'block_id')})),[meta]);const save=()=>{editing?update(editing,f):submit(f);setEditing(null);setF(initial());};const edit=(r:any)=>{setEditing(r.rain_id);setF({block_id:String(r.block_id),recorded_date:r.recorded_date,rain_value:String(r.rain_value),created_by:'Admin'});};return <><Form title={`${editing?'Edit':'Add'} rainfall`} onSubmit={save}><Select label="Block from selected property" v="block_id" f={f} setF={setF} opts={meta?.blocks} id="block_id" name="block_name"/><Input label="Date" type="date" v="recorded_date" f={f} setF={setF}/><Input label="Rain mm" type="number" step="0.1" v="rain_value" f={f} setF={setF}/>{editing&&<button type="button" className="secondary" onClick={()=>{setEditing(null);setF(initial());}}>Cancel edit</button>}</Form><Table rows={rows} columns={['recorded_date','block_name','rain_value','baseunit_name','property_name']} onEdit={edit} onDelete={(r:any)=>remove(r.rain_id)}/></>}
function Yield({meta,rows,submit,update,remove}:any){const initial=()=>({yieldrate_id:firstId(meta?.yieldRates,'yieldrate_id'),picking_date:today,quantity:'0',created_by:'Admin'});const [f,setF]=useState(initial);const [editing,setEditing]=useState<any>(null);useEffect(()=>setF((x:any)=>({...x,yieldrate_id:x.yieldrate_id||firstId(meta?.yieldRates,'yieldrate_id')})),[meta]);const save=()=>{editing?update(editing,f):submit(f);setEditing(null);setF(initial());};const edit=(r:any)=>{setEditing(r.yieldsettlement_id);setF({yieldrate_id:String(r.yieldrate_id),picking_date:r.picking_date,quantity:String(r.quantity),created_by:'Admin'});};return <><Form title={`${editing?'Edit':'Add'} yield settlement`} onSubmit={save}><Select label="Yield rate from selected property" v="yieldrate_id" f={f} setF={setF} opts={meta?.yieldRates} id="yieldrate_id" name="season"/><Input label="Picking date" type="date" v="picking_date" f={f} setF={setF}/><Input label="Quantity" type="number" step="0.1" v="quantity" f={f} setF={setF}/>{editing&&<button type="button" className="secondary" onClick={()=>{setEditing(null);setF(initial());}}>Cancel edit</button>}</Form><Table rows={rows} columns={['picking_date','yieldrate_name','yieldtype_name','plant_name','quantity','rate','baseunit_name','property_name','estimated_value']} onEdit={edit} onDelete={(r:any)=>remove(r.yieldsettlement_id)}/></>}
...
function Crud({resource,meta,rows,save,update,remove}:any){
  ...
  return <>
    ...
    <Table rows={namedRows(resource,rows,meta)} columns={gridColumns[resource]} onEdit={idKey ? startEdit : undefined} onDelete={idKey ? (r:any)=>remove(resource,r[idKey]) : undefined}/>
  </> }
...
function Input({label,v,f,setF,...props}:any){if(props.type==='checkbox')return <label><span className="labelText">Status</span><span className="checkField"><input type="checkbox" checked={String(f[v]).toLowerCase()==='active'} onChange={e=>setF({...f,[v]:e.target.checked?'active':'inactive'})}/><strong>{label}</strong></span></label>;return <label><span className="labelText">{label}<InfoTip text={`Enter ${String(label).toLowerCase()}`}/></span><input {...props} value={f[v] ?? ''} onChange={e=>setF({...f,[v]:e.target.value})}/></label>}
...
function Table({rows=[],onDelete,onEdit,columns}:any){ const visibleRows = useMemo(()=>rows || [],[rows]); if(!visibleRows?.length) return <p className="empty">No data yet.</p>; const keys=(columns||Object.keys(visibleRows[0])).filter((k:string)=>!String(k).includes('password') && !String(k).includes('photo') && !String(k).includes('adhar') && !String(k).includes('bank')); return <div className="table"><table><thead><tr>{keys.map((k:string)=><th key={k}>{k.replaceAll('_',' ')}</th>)}{(onDelete||onEdit)&&<th>Action</th>}</tr></thead><tbody>{visibleRows.map((r:any,i:number)=><tr key={i}>{keys.map((k:string)=><td key={k} title={String(r[k] ?? '')}>{String(r[k] ?? '')}</td>)}{(onDelete||onEdit)&&<td className="actions">{onEdit&&<button type="button" className="secondary" onClick={()=>onEdit(r)}>Edit</button>}{onDelete&&<button className="danger" onClick={()=>onDelete(r)}>Delete</button>}</td>}</tr>)}</tbody></table></div>}
