...
export const updateQuickEntry = createAsyncThunk('app/updateQuickEntry', async ({ resource, id, payload }: any, { dispatch }) => { await api(`/api/${resource}/${id}`, { method: 'PATCH', body: JSON.stringify(payload) }); const loader = resource === 'attendance' ? loadAttendance : resource === 'rainfall' ? loadRainfall : loadYield; dispatch(loader()); dispatch(loadDashboard()); });
export const deleteQuickEntry = createAsyncThunk('app/deleteQuickEntry', async ({ resource, id }: any, { dispatch }) => { await api(`/api/${resource}/${id}`, { method: 'DELETE' }); const loader = resource === 'attendance' ? loadAttendance : resource === 'rainfall' ? loadRainfall : loadYield; dispatch(loader()); dispatch(loadDashboard()); });
