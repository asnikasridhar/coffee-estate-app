...
function applyProperty(resource, payload, propertyId, userId) {
  ...
  if (['blocks','plantInventory','assets','inventory','expenses','cropDetails','fertilizers','reports'].includes(resource)) return { ...payload, property_id: Number(payload.property_id || propertyId) };
  return payload;
}
