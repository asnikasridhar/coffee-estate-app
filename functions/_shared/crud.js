...
function applyProperty(resource, payload, propertyId, userId) {
  ...
  if (['blocks','plants','crops','plantInventory','workActivities','workAssignments','assets','inventory','expenses','cropDetails','fertilizers','reports'].includes(resource)) return { ...payload, property_id: Number(payload.property_id || propertyId) };
  return payload;
}
