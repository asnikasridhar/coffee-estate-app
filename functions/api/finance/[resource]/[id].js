...
import { financeContext, season, crop, yieldType } from '../../../_shared/finance.js';
...
async function patchFinance({request,env,params}){try{const {propertyId}=await financeContext(request,env);const b=await body(request),id=Number(params.id),who=String(b.modified_by||'Owner').slice(0,80);
  ...
  if(!b.action&&['seasons','cycles','wageRules','commissionRules','yieldTypes','marketRates','buyerOffers'].includes(params.resource)){
    ...
    else if(params.resource==='commissionRules'){stmt=`UPDATE finance_vendor_commission_rule SET season_id=?,labour_engagement_id=?,effective_from=?,effective_to=?,commission_percentage=?,status=?,modified_on=CURRENT_TIMESTAMP,modified_by=? WHERE vendor_commission_rule_id=? AND property_id=? AND EXISTS(SELECT 1 FROM finance_labour_engagement WHERE labour_engagement_id=? AND property_id=? AND labour_type='vendor')`;values=[b.season_id||null,b.labour_engagement_id,b.effective_from,b.effective_to||null,Number(b.commission_percentage),b.status||'active',who,id,propertyId,b.labour_engagement_id,propertyId];}
