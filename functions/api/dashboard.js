...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const plantInventorySummary = await first(env, `SELECT COUNT(*) entries, COALESCE(SUM(plant_count),0) total_plants FROM plant_inventory WHERE property_id = ?`, propertyId);
    const plantByBlock = await all(env, `SELECT b.block_name AS name, COALESCE(SUM(pi.plant_count),0) value FROM blocks b LEFT JOIN plant_inventory pi ON pi.block_id = b.block_id AND pi.property_id = ? WHERE b.property_id = ? GROUP BY b.block_id, b.block_name HAVING value > 0 ORDER BY value DESC`, propertyId, propertyId);
    const plantByType = await all(env, `SELECT pd.plant_type AS name, COALESCE(SUM(pi.plant_count),0) value FROM plant_inventory pi JOIN plantdetails pd ON pd.plant_id = pi.plant_id WHERE pi.property_id = ? GROUP BY pd.plant_id, pd.plant_type ORDER BY value DESC`, propertyId);
    const plantBySubBlock = await all(env, `SELECT b.block_name, COALESCE(pi.sub_block_name,'Unspecified') AS sub_block_name, COALESCE(SUM(pi.plant_count),0) plant_count FROM plant_inventory pi JOIN blocks b ON b.block_id = pi.block_id WHERE pi.property_id = ? GROUP BY b.block_name, COALESCE(pi.sub_block_name,'Unspecified') ORDER BY b.block_name, plant_count DESC`, propertyId);
    return json({ plantInventorySummary, plantByBlock, plantByType, plantBySubBlock, ranges: { attendance: attendanceRange, rainfall: rainfallRange, yield: yieldRange, expenses: expenseRange, income: incomeRange, assets: assetRange, profit: profitRange, recent: recentRange }, attendance, laborCost, rainfall, yieldTotal, expenses, income, assets, profit, recentAttendance, rainByBlock, propertyProfit });
